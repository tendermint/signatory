...
where
    S: Signature,
    T: DigestSigner<Sha256, S, DigestSize = U32>,
{
    fn sign_sha256(&self, msg: &[u8]) -> Result<S, Error> {
...
where
    S: Signature,
    T: DigestSigner<Sha384, S, DigestSize = U48>,
{
    fn sign_sha384(&self, msg: &[u8]) -> Result<S, Error> {
...
where
    S: Signature,
    T: DigestSigner<Sha512, S, DigestSize = U64>,
{
    fn sign_sha512(&self, msg: &[u8]) -> Result<S, Error> {
...
/// This can be used to avoid importing the `Signer` and `Signature` traits
pub fn sign_sha384<M, S>(signer: &Sha384Signer<S>, msg: &[u8]) -> Result<S, Error>
where
    S: Signature,
{
...
/// This can be used to avoid importing the `Signer` and `Signature` traits
pub fn sign_sha512<M, S>(signer: &Sha512Signer<S>, msg: &[u8]) -> Result<S, Error>
where
    S: Signature,
{
